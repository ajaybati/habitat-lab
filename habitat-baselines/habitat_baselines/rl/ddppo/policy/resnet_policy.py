...
class PointNavResNetNet(Net):
    ...
    def __init__(
        ...
        if ObjectGoalPromptSensor.cls_uuid in observation_space.spaces:
            ...
            #     int(
            #         observation_space.spaces[ObjectGoalPromptSensor.cls_uuid].high[0]
            #     )
            #     + 1
            # )
    ...
    def forward(
        ...
        if ObjectGoalPromptSensor.cls_uuid in observations:
            object_goal = observations[ObjectGoalPromptSensor.cls_uuid]
            x.append(object_goal) #self.obj_categories_embedding(object_goal).squeeze(dim=1)
